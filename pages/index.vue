<template>
  ...
  <div class="space-y-6">
    ...
    <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-blue-600 to-blue-800 p-6 lg:p-8 text-white shadow-lg">
      <div class="absolute top-0 right-0 w-64 h-64 bg-white/5 rounded-full -mr-20 -mt-20">

      </div>

    
      <div class="absolute bottom-0 right-32 w-32 h-32 bg-white/5 rounded-full -mb-12">
      ...
      <div class="relative">
        ...
        <div class="mt-5 flex flex-wrap gap-3">
          ...
          <NuxtLink
  to="/classes"
  class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white/10 text-white text-sm font-semibold hover:bg-white/20 border border-white/20 transition-colors"
>
  <svg
    class="w-4 h-4"
    fill="none"
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-4">
        ...
        <template v-if="loading">
          <div class="p-5 rounded-xl bg-slate-100 h-[84px] animate-pulse">
...
`${subjects.filter(s => s.status === "active").length} active`
    ...
    feeStats.value.collected =
      fees.reduce(
        (sum, fee) =>
          sum + Number(fee.paid_amount),
        0
      )

    feeStats.value.pending =
      fees.reduce(
        (sum, fee) =>
          sum +
          (
...
stats.value[4].trend =
formatCurrency(feeStats.value.pending)
